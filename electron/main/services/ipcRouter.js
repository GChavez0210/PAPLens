...
class IpcRouter {
  ...
  get cpap() {
    return this.appContainer.get("cpapService");
  }
  get profileDb() {
    return this.appContainer.get("profileDatabase");
  }
  get appDb() {
    return this.appContainer.get("appDatabase");
  }
  get secureSettings() {
    return this.appContainer.get("secureSettings");
  }
  get windowManager() {
    return this.appContainer.get("windowManager");
  }
  ...
  register() {
    ipcMain.handle("cpap:select-data-folder", async () => {
      ...
      if (!detectDataFolder(selectedPath)) {
        return {
          success: false,
          error:
            "No supported CPAP data found. Supported devices: ResMed AirSense/AirCurve, Resvent iBreezer, DeVilbiss IntelliPAP, Fisher & Paykel SleepStyle/ICON, Lowenstein/Weinmann WM_DATA, Löwenstein Prisma Line, Apex Medical, BMC/3B RESmart, and Yuwell."
        };
      }
    ...
    ipcMain.handle("cpap:get-best-session-for-date", async (_event, date) => {
      ...
      const session = this.cpap.dataLoader.sessions.find((s) => s.date === compact || s.date === prevCompact);
    ...
    ipcMain.handle("cpap:save-report", async (_event, reportData) => {
      ...
      try {
        ...
        const templatePath = isDev
          ...
          : path.join(process.resourcesPath, "app.asar", "report.html");
        ...
        if (this.profileDb && reportData && reportData.device) {
          ...
          const deviceId = db
            ...
            .get(reportData.device.serialNumber)?.id;

          if (deviceId) {
            const logoPath = this.resolveAssetPath(path.join("Assets", "PAPLens Logo Light.png"));
            let logoDataUri = "";
            try {
              logoDataUri = `data:image/png;base64,${fs.readFileSync(logoPath).toString("base64")}`;
            } catch (err) {
              console.error("Printable PAPLens logo not found", err);
            }
        ...
        const tempHtmlPath = path.join(
          app.getPath("temp"),
          `paplens-report-${Date.now()}-${Math.random().toString(16).slice(2)}.html`
        );
        ...
        try {
          ...
          fs.writeFileSync(
            filePath,
            await printWin.webContents.printToPDF({
    ...
    ipcMain.handle("app:get-last-data-path", async () => {
      return (
        this.cpap.currentDataPath ||
        this.cpap.getLatestImportedPath() ||
        this.secureSettings.getDecrypted("lastDataPath")
      );
    });
    ...
    ipcMain.handle("cpap:get-last-night-overview", async () => {
      ...
      const lastNight = this.profileDb.db
        .prepare(
          `
      ...
      const history30 = this.profileDb.db
        .prepare(
          `
    ...
    ipcMain.handle("cpap:get-insights", async (_event, payload) => {
      ...
      const complianceWindow = this.profileDb.db
        .prepare(
          `
      ...
      const complianceRate =
        complianceWindow.length > 0 ? Math.round((compliantNights / complianceWindow.length) * 100) : null;

      const latestCorrs = this.profileDb.db
        .prepare(
          `
      ...
      const lastNightDate = this.profileDb.db
        .prepare(
          `
    ...
    ipcMain.handle("app:get-profiles", () => {
      return this.appDb.getProfiles();
    });
    ipcMain.handle("app:create-profile", (_event, { id, name, age, notes }) => {
      this.appDb.createProfile(id, name, age, notes);
      return { success: true };
    });
    ipcMain.handle("app:delete-profile", async (_event, profileId) => {
      ...
      try {
        ...
        if (profileId === activeId) {
          if (this.profileDb) {
            this.profileDb.close();
            this.appContainer.register("profileDatabase", null);
          }
          this.appContainer.register("activeProfileId", null);
          this.cpap.currentSummary = null;
          this.cpap.currentDataPath = null;
          this.cpap.dataLoader = null;
