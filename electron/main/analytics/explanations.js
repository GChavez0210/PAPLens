...
const {
  ...
  RIN_ELEVATED
} = require("../../../src/shared/clinicalThresholds");
...
function describeFlagInContext(metric, z) {
  ...
  switch (metric) {
    case "usage_hours":
      return higher ? "therapy usage was longer than usual" : "therapy usage was shorter than usual — check adherence";
    ...
    case "tidal_vol_p50":
      return higher ? "tidal volume was above your recent baseline" : "tidal volume was below your recent baseline";
...
function generatePeriodicBreathingInsight(pbPct, pbIsSignificant, episodeCount, leakConfounded) {
  ...
  if (pbIsSignificant) {
    ...
    return {
      ...
      summary: `Cheyne-Stokes–like breathing pattern occupied ${pbPct.toFixed(1)}% of that night's recording (${episodeCount} episode${episodeCount !== 1 ? "s" : ""}). This cyclic waxing-and-waning pattern can indicate central respiratory instability and warrants discussion with your care team.`,
      details: null
    };
  }
  if (pbPct >= 2) {
...
function generateFlowLimitationInsight(flP95, rin, nightsElevated) {
  ...
  const severeFL = flP95 >= FLOW_LIMITATION_SIGNIFICANT;
  const mildFL = flP95 >= FLOW_LIMITATION_MILD;
  const elevatedRin = rin != null && rin > RIN_ELEVATED;

  if (!mildFL && !elevatedRin) return null;

  if (severeFL) {
    const rinNote = elevatedRin
      ? ` Combined with an RIN of ${rin.toFixed(1)} events/hr, this suggests repeated partial airway obstruction that your device may not be fully resolving.`
      : "";
  ...
  if (mildFL) {
    const rinNote = elevatedRin
      ? ` RIN of ${rin.toFixed(1)} events/hr also indicates flow-limited breaths that fell below the hypopnea scoring threshold.`
      : "";
...
function generateInsightNarratives(nightId, derivedMetrics, lastNightFlags) {
  ...
  if (derivedMetrics.stability_score !== null) {
    ...
    if (derivedMetrics.stability_score < STABILITY_UNSTABLE) {
  ...
  if (derivedMetrics.mask_fit_score !== null) {
    ...
    if (derivedMetrics.mask_fit_score < MASK_FIT_POOR) {
      title = "Mask Fit Issues";
      summary = "Significant mask leaking was detected, potentially compromising therapy.";
    } else if (derivedMetrics.mask_fit_score < MASK_FIT_MODERATE) {
  ...
  if (derivedMetrics.compliance_risk === "high") {
    insights.push({
      ...
      summary:
        "You averaged less than 4 hours of therapy use across the last 14 nights. Consistency is key to effective treatment.",
      details: null
    });
    ...
    insights.push({
      ...
      title: "Usage Warning",
      summary:
        "Your usage across the last 14 nights is dipping close to the 4-hour minimum threshold. Try to maintain consistent therapy.",
      details: null
    });
  }
