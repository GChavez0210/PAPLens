...
const PB_DEPTH_THRESHOLD = 0.6; // 60% amplitude/mean ratio — raises bar above normal breath-to-breath variability
...
const LEAK_CONFOUND_TROUGH_RATIO = 0.5; // ≥50% of troughs must coincide with elevated leak
const TROUGH_AMPLITUDE_RATIO = 0.5; // trough = sample below 50% of episode mean
...
function isLeakConfoundedEpisode(smoothedSlice, leakSamples, startIdx, leakThreshold) {
  ...
  for (let i = 0; i < smoothedSlice.length; i++) {
    if (smoothedSlice[i] <= troughThreshold) {
      ...
      for (let j = lo; j <= hi; j++) {
        if (leakSamples[j] > leakThreshold) {
          leakCoincidentCount++;
          break;
        }
      }
...
function detectPeriodicBreathing(tidalMlSamples, leakSamples) {
  ...
  // Round before deciding significance, not after. Testing the raw value and
  // storing the rounded one let a 4.96% night persist as pb_pct = 5.0 with
  // pb_is_significant = 0, so the UI showed "5.0% — below the clinical
  // significance threshold" and coloured it as if it were under 5%.
  const pbPct = totalSec > 0 ? Math.round((totalPBSec / totalSec) * 1000) / 10 : 0;
...
module.exports = { detectPeriodicBreathing, mergeEpisodes, CLINICAL_SIGNIFICANCE_PCT };
